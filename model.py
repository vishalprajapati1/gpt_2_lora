...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.c_attn_aux = LoRALayer(config.n_embd, 3 * config.n_embd, config.LoRA_rank)
        self.c_proj_aux = LoRALayer(config.n_embd, config.n_embd, config.LoRA_rank)
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.c_fc_aux = LoRALayer(config.n_embd, 4 * config.n_embd, config.LoRA_rank)
        self.c_proj_aux = LoRALayer(4 * config.n_embd, config.n_embd, config.LoRA_rank)
...
class LoRALayer(torch.nn.Module):
    def __init__(self, in_dim, out_dim, rank):
        super().__init__()
        self.A = torch.nn.Parameter(torch.randn(in_dim, rank) / math.sqrt(rank))
